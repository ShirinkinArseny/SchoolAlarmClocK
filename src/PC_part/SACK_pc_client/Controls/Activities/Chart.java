        ...
        for (int i=0; i<7; i++) {
            ...
            for (Ring r: rigns) {
                ...
                if (r.getHours()>=9 && r.getHours()<=19) {
                    int y = (int) ((r.getHours() - 7) * PC_part.SACK_pc_client.Controls.Menu.itemHeight + (Images.backArrow.getHeight() - Images.markBoards.getHeight()) / 2);
                    g2.drawImage(Images.markBoards, PC_part.SACK_pc_client.Controls.Menu.itemWidth + i * WeekDaysPanel.dayWidth + markXOffset - Images.markBoards.getWidth() / 2, y, null);
                }
            }
